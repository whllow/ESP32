...
const long  gmtOffset_sec = 4*3600;//这里采用UTC计时，中国为东八区，就是 8*60*60
const int   daylightOffset_sec = 4*3600;//同上
...
void setup(void)
{
  ...
  sensors.begin();
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);//连接时间服务器
}
...
struct tm tinfo;
  ...
  //构建传感器字符串的参数 
    ...
    root["deviceId"] = "Esp32A004";
    root["deviceTime"] = times;
    root["ph"] = ph;
    root["tds"] = tds;
